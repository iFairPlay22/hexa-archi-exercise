...
In order to run the application, you need to install `java`, `quarkus-cli` and `docker` in your machine.
...
- https://docs.docker.com/desktop (for docker)

Then, start docker and run the backend in dev using:
```shell script
quarkus dev
```

The application will be available in port **8080**, and a MongoDB webservice will be automatically launched in port **27017**.
